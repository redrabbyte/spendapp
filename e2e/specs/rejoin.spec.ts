...
test('a returning member is told their name comes back, not asked to choose', async ({ page, api }) => {
  ...
  await expect(page.getByRole('option', { name: `No — rejoin as ${ME.displayName}` })).toBeAttached();
...
test('someone who joined as the wrong person can still pick a different name', async ({ page, api }) => {
  ...
  // The whole point of leaving and coming back: choose again.
  await expect(page.getByText(/Taking over somebody/)).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Join as this person' }).click();
